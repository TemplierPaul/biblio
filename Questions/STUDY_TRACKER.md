# Study Tracker - Interview Preparation Checklist

Track your progress by File and Part. Focus on mastering one section at a time. The goal is depth, not just checking boxes.

---

## 🤖 Machine Learning & AI

### [[01_Math_Foundations]]
- [x] Part 1: Optimization Theory
- [x] Part 2: Linear Algebra
- [ ] Part 3: Tensors & Decompositions

### [[02_Probability_Statistics]]
- [x] Part 1: Probability Fundamentals
- [x] Part 2: Distributions
- [x] Part 3: Bayesian Inference
- [ ] Part 4: Statistical Testing
- [ ] Part 5: Bias-Variance Tradeoff
- [ ] Part 6: Information Theory & Divergence

### [[03_ML_Fundamentals]]
- [ ] Part 1: Evaluation Metrics
- [ ] Part 2: Model Debugging & Improvement

### [[04_Loss_Functions]]
- [ ] Part 1: Core Supervised & Representation Learning
- [ ] Part 2: Generative & Probabilistic Objectives
- [ ] Part 3: Reinforcement Learning & Control
- [ ] Part 4: LLM Alignment
- [ ] Part 5: Synthesis & Quick Reference

### [[05_Optimizers_Regularization]]
- [ ] Part 1: Optimizers
- [ ] Part 2: Regularization & Overfitting

### [[06_Classical_ML]]
- [ ] Part 1: Classification & Regression Models
- [ ] Part 2: Gaussian Processes

### [[07_Neural_Architectures]]
- [ ] Part 1: Convolutional Neural Networks (CNNs)
- [ ] Part 2: LSTM
- [ ] Part 3: Attention Mechanism
- [ ] Part 4: Transformer Architecture
- [ ] Part 5: Graph Neural Networks

### [[08_Advanced_Deep_Learning]]
- [ ] Part 1: Generative Models
- [ ] Part 2: Diffusion Models
- [ ] Part 3: Vision-Language-Action Models
- [ ] Part 4: Advanced Architectures

### [[09_LLM_Training]]
- [ ] Part 1: Large Language Models
- [ ] Part 2: Fine-tuning & PEFT
- [ ] Part 3: Alignment & RLHF

### [[10_Reinforcement_Learning]]
- [x] Part 1: MDP Foundations
- [x] Part 2: Value Functions & Bellman Equations
- [x] Part 3: Dynamic Programming
- [x] Part 4: Monte Carlo Methods
- [ ] Part 5: Temporal Difference Learning
- [ ] Part 6: Function Approximation
- [ ] Part 7: Policy Gradient Methods
- [ ] Part 8: Actor-Critic Methods
- [ ] Part 9: Deep Q-Networks (DQN)
- [ ] Part 10: Continuous Control
- [ ] Part 11: Advanced Topics
- [ ] Part 12: Practical Considerations

### [[11_Game_Theory]]
- [x] Part 1: Game Theory Foundations
- [x] Part 2: Social Dilemmas
- [x] Part 3: Classical Game Algorithms
- [ ] Part 4: AlphaZero
- [x] Part 5: PSRO & Population-Based Methods
- [ ] Part 6: Neural Population Learning
- [ ] Part 7: Fictitious Self-Play
- [ ] Part 8: Counterfactual Regret Minimization

### [[12_ML_Systems]]
- [ ] Part 1: Scalable ML Systems
- [ ] Part 2: Recommendation Systems

### [[13_Ethics_AI]]
- [ ] Part 1: Ethical Considerations in AI
- [ ] Part 2: Bias Mitigation
- [ ] Part 3: Practical Considerations
- [ ] Part 4: Interview Questions & Answers

---

## 💻 Computer Science Fundamentals

### [[01_Data_Structures]]
- [x] Part 1: Arrays & Strings
- [x] Part 2: Linked Lists
- [ ] Part 3: Stacks & Queues
- [ ] Part 4: Trees
- [ ] Part 5: Heaps & Priority Queues
- [x] Part 6: Hash Tables
- [ ] Part 7: Graphs
- [ ] Part 8: Tries

### [[02_Algorithms]]
- [ ] Part 1: Sorting Algorithms
- [ ] Part 2: Searching Algorithms
- [ ] Part 3: Dynamic Programming
- [ ] Part 4: Greedy Algorithms
- [ ] Part 5: Backtracking
- [ ] Part 6: Complexity Analysis

### [[03_Systems_Programming]]
- [ ] Part 1: Memory Management
- [ ] Part 2: Operating Systems
- [ ] Part 3: Compilation & Linking
- [ ] Part 4: Computer Architecture
- [ ] Part 5: Concurrency

### [[04_Programming_Languages]]
- [x] Part 1: Python vs C++
- [x] Part 2: Object-Oriented Programming
- [x] Part 3: Type Systems
- [x] Part 4: Advanced Concepts

### [[05_Distributed_Systems]]
- [ ] Part 1: Distributed Computing Fundamentals
- [ ] Part 2: MapReduce & Spark
- [ ] Part 3: Distributed Training
- [ ] Part 4: Fault Tolerance & Reliability
- [ ] Part 5: Communication Patterns

### [[06_System_Design]]
- [x] Part 1: ML System Design Framework
- [ ] Part 2: Model Serving & Deployment
- [ ] Part 3: Monitoring & Maintenance
- [ ] Part 4: Case Studies

### [[07_Databases_SQL]]
- [ ] Part 1: SQL Fundamentals
- [ ] Part 2: Database Design
- [ ] Part 3: NoSQL Databases
- [ ] Part 4: Optimization & Internals

### [[08_ML_Infrastructure]]
- [ ] Part 1: GPU Programming & CUDA
- [ ] Part 2: Model Optimization
- [ ] Part 3: MLOps & Infrastructure
- [ ] Part 4: Serving Frameworks

### [[09_Software_Engineering]]
- [ ] Part 1: Git & Version Control
- [ ] Part 2: Testing & CI/CD
- [ ] Part 3: Code Quality & Design
- [ ] Part 4: Performance Profiling
